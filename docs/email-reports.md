...
- `05:30 UTC` ежедневно (**08:30 МСК**) — дневной отчёт за вчера; по понедельникам также недельный;
- `06:00 UTC` (**09:00 МСК**) — резервная попытка, если импорт или GitHub Actions задержались;
- после успешного **Sync Drive Reports** — рассылка с ожиданием до **08:30 МСК** (данные уже в БД).
