...
async function chunkedInsert(supabase, table, rows, batchSize = 500) {
  if (rows.length === 0) {
    return
  }
...
async function chunkedUpsert(supabase, table, rows, onConflict, batchSize = 500) {
  if (rows.length === 0) {
    return
  }
...
async function fetchLookupMap(supabase, table, keyColumn, values) {
...
function buildImportFileRecords(batchId, reportDate, files) {
...
function mapLongIdleFactRows(batchId, rows) {
  return rows.map((row) => ({
    batch_id: batchId,
...
function validateLongIdleRows(rows) {
...
function buildFaceRowsFromLongIdle(rows) {
  ...
  return [...byShift.entries()].map(([wwShiftId, shiftRows]) => {
    ...
    const techSessionIds = [
      ...new Set(shiftRows.map((row) => row.tech_session_id).filter((id) => Number.isFinite(id))),
    ]
...
function resolveFaceRows(faceRows, longIdleRows) {
  if (faceRows.length > 0) return faceRows
  return buildFaceRowsFromLongIdle(longIdleRows)
}

export async function importDailyBatch(supabase, options) {
  const {
    reportDate,
    sourceDayKey,
    notes,
  ...
  const { data: shiftData, error: shiftSelectError } = await supabase
    ...
    .in(
      'ww_shift_id',
      shiftRows.map((row) => Number(row.ww_shift_id)),
    )
...
export async function markImportBatchFailed(supabase, reportDate, sourceDayKey, errorMessage) {
...
export async function getReadyBatch(supabase, reportDate, sourceDayKey) {
  const { data, error } = await supabase
    .from('import_batches')
    .select('id, status')
    .eq('report_date', reportDate)
...
export async function getBatchImportFiles(supabase, batchId) {
  const { data, error } = await supabase
    .from('import_files')
