...
export const METRIC_DEFINITIONS: MetricDefinition[] = [
  ...
  },
  {
    ...
    formula:
      'Подозрительная минута в учитываемой зоне (не отдых, не zona 0, не обед): idle_sec ≥ порог И work_sec + go_sec ≤ порог\nВ метрику попадают только непрерывные эпизоды ≥ 30 мин\nnot_worn_pct = 100 × not_worn_sec / eligible_sec',
    configFields: [
      {
      ...
      },
      {
        ...
        min: 60,
        max: 14400,
        hint: 'Считать только если подозрительный простой идёт непрерывно не меньше этого времени (по умолчанию 1800 = 30 мин)',
      },
      {
        key: 'notWornMinSec',
    ...
    notes: 'Список сотрудников — в блоке 1 «Ежедневная аналитика». В рассылку не входит. Пороги по профессиям — в настройках блока 1.',
  },
]
