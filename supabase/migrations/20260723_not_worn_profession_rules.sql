-- Пороги «Не носил» по профессиям (JSON overrides + fallback на глобальные колонки).

alter table analytics.metric_settings
  add column if not exists not_worn_profession_rules jsonb not null default '{}'::jsonb;

create or replace function analytics.not_worn_profession_rule_int(
  p_profession text,
  p_field text,
  p_global int,
  p_fallback int
  ...
  select coalesce(
    nullif(
      trim(
        coalesce(
          (
            select not_worn_profession_rules
            from analytics.metric_settings
            order by id
            limit 1
          )->nullif(trim(p_profession), '')->>p_field,
          ''
        )
      ),
      ''
...
create or replace function analytics.not_worn_idle_sec_min_for(p_profession text)
returns integer
language sql
stable
as $$
  select analytics.not_worn_profession_rule_int(
    p_profession,
    'idle_sec_min',
    analytics.not_worn_idle_sec_min(),
    54
  );
$$;

create or replace function analytics.not_worn_active_sec_max_for(p_profession text)
returns integer
language sql
stable
as $$
  select analytics.not_worn_profession_rule_int(
    p_profession,
    'active_sec_max',
    analytics.not_worn_active_sec_max(),
    6
  );
$$;

create or replace function analytics.not_worn_min_sec_for(p_profession text)
returns integer
language sql
stable
as $$
  select analytics.not_worn_profession_rule_int(
    p_profession,
    'shift_min_sec',
    analytics.not_worn_min_sec(),
    900
  );
$$;

create or replace function analytics.not_worn_warn_pct_for(p_profession text)
returns integer
language sql
stable
as $$
  select analytics.not_worn_profession_rule_int(
    p_profession,
    'warn_pct',
    coalesce((select not_worn_warn_pct from analytics.metric_settings order by id limit 1), 5),
    5
  );
$$;
