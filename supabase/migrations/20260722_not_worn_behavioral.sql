-- «Не носил»: поведенческий критерий по AA_BLE (без датчика wear).
-- Минута считается подозрительной вне зон отдыха, если idle_sec высокий, а work+go почти нулевые.

alter table analytics.metric_settings
  add column if not exists not_worn_idle_sec_min integer not null default 54,
  add column if not exists not_worn_active_sec_max integer not null default 6;

create or replace function analytics.not_worn_idle_sec_min()
returns integer
language sql
stable
as $$
  select coalesce(
    (select not_worn_idle_sec_min from analytics.metric_settings order by id limit 1),
    54
  );
$$;

create or replace function analytics.not_worn_active_sec_max()
returns integer
language sql
stable
as $$
  select coalesce(
    (select not_worn_active_sec_max from analytics.metric_settings order by id limit 1),
    6
  );
$$;
...
as $$
  select not analytics.is_rest_zone(p_zona)
    ...
    and coalesce(p_idle_sec, 0) >= analytics.not_worn_idle_sec_min()
    and coalesce(p_work_sec, 0) + coalesce(p_go_sec, 0) <= analytics.not_worn_active_sec_max();
$$;
