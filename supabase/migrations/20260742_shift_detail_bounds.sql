-- Детализация смены: фактические границы смены — получение и сдача часов (отчёт 6).
-- Только факт: watch_received_at / watch_returned_at. Плановые времена и края телеметрии
-- НЕ используем — это догадки, а метка границы смены должна быть железобетонной.
-- Если факта нет (часы не сданы) — возвращаем null, и метка просто не рисуется.
